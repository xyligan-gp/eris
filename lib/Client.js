...
class Client extends EventEmitter {
	...
	banGuildMember(guildID, userID, options, reason) {
		...
		if (
			...
			!Object.hasOwnProperty.call(options, 'deleteMessageSeconds')
		) {
			options.deleteMessageSeconds = options.deleteMessageDays * 24 * 60 * 60;
			emitDeprecation('DELETE_MESSAGE_DAYS');
			this.emit(
				'warn',
				'[DEPRECATED] banGuildMember() was called with the deleteMessageDays argument'
			);
		}
		return this.requestHandler.request(
			'PUT',
	...
	bulkBanGuildMembers(guildID, options) {
		return this.requestHandler.request(
			'POST',
			Endpoints.GUILD_BULK_BAN(guildID),
			true,
			{
		...
		for (const command of commands) {
			if (command.name !== undefined) {
				if (command.type === 1 || command.type === undefined) {
					...
					if (
						!command.name.match(/^[-_\p{L}\p{N}\p{sc=Deva}\p{sc=Thai}]{1,32}$/u)
					) {
						throw new Error(
							'Slash Command names must match the regular expression "^[-_\\p{L}\\p{N}\\p{sc=Deva}\\p{sc=Thai}]{1,32}$"'
						);
					}
				}
			}
			if (command.defaultPermission !== undefined) {
				...
				this.emit(
					'warn',
					'[DEPRECATED] bulkEditCommands() was called with a `defaultPermission` parameter. This has been replaced with `defaultMemberPermissions`'
				);
			...
			if (command.defaultMemberPermissions !== undefined) {
				command.default_member_permissions =
					command.defaultMemberPermissions === null
						? null
						: String(
								command.defaultMemberPermissions.allow ||
			...
			for (let i = 0; i < command.options?.length; i++) {
				if (command.options[i].descriptionLocalizations !== undefined) {
					command.options[i].description_localizations = command.options[i].descriptionLocalizations;
				}
			}
		}
		return this.requestHandler
			.request('PUT', Endpoints.COMMANDS(this.application.id), true, commands)
			.then(commands =>
				commands.map(command => new ApplicationCommand(command, this))
			);
	}
	...
	bulkEditGuildCommands(guildID, commands) {
		for (const command of commands) {
			...
			if (command.defaultPermission !== undefined) {
				...
				this.emit(
					'warn',
					'[DEPRECATED] bulkEditGuildCommands() was called with a `defaultPermission` parameter. This has been replaced with `defaultMemberPermissions`'
				);
				command.default_permission = command.defaultPermission;
			}
	...
	closeVoiceConnection(guildID) {
		this.shards
			.get(this.guildShardMap[guildID] || 0)
	...
	async connect() {
		...
		try {
			...
			(this.options.shardConcurrency === 'auto' && this.bot)
				? this.getBotGateway()
				: this.getGateway());
			...
			this.gatewayURL = `${data.url}?v=${Constants.GATEWAY_VERSION}&encoding=${
				Erlpack ? 'etf' : 'json'
			}`;
	...
	createAutoModerationRule(guildID, options) {
		return this.requestHandler
			.request('POST', Endpoints.AUTO_MODERATION_RULES(guildID), true, {
	...
	createChannel(guildID, name, type, reason, options = {}) {
		if (typeof options === 'string') {
			// This used to be parentID, back-compat
			emitDeprecation('CREATE_CHANNEL_OPTIONS');
			this.emit(
				'warn',
				'[DEPRECATED] createChannel() was called with a string `options` argument'
			);
			options = {
				parentID: options,
			};
		...
		if (typeof reason === 'string') {
			// Reason is deprecated, will be folded into options
			emitDeprecation('CREATE_CHANNEL_OPTIONS');
			this.emit(
				'warn',
				'[DEPRECATED] createChannel() was called with a string `reason` argument'
			);
			options.reason = reason;
			reason = undefined;
	...
	createCommand(command) {
		...
		if (command.defaultPermission !== undefined) {
			...
			this.emit(
				'warn',
				'[DEPRECATED] createCommand() was called with a `defaultPermission` parameter. This has been replaced with `defaultMemberPermissions`'
			);
			command.default_permission = command.defaultPermission;
		}
		...
		return this.requestHandler
			.request('POST', Endpoints.COMMANDS(this.application.id), true, command)
	...
	createEmoji(options) {
		return this.requestHandler.request(
			'POST',
			Endpoints.APPLICATION_EMOJIS(this.application.id),
			true,
			options
		);
	}
	...
	createGuildCommand(guildID, command) {
		...
		if (command.defaultPermission !== undefined) {
			...
			this.emit(
				'warn',
				'[DEPRECATED] createGuildCommand() was called with a `defaultPermission` parameter. This has been replaced with `defaultMemberPermissions`'
			);
			command.default_permission = command.defaultPermission;
		}
		if (command.descriptionLocalizations !== undefined) {
	...
	createGuildSoundboardSound(guildID, sound, reason) {
		...
		sound.reason = reason;
		return this.requestHandler
			.request('POST', Endpoints.SOUNDBOARD_SOUNDS_GUILD(guildID), true, sound)
	...
	createInteractionResponse(interactionID, interactionToken, options, file) {
		...
		if (options.data && options.data.allowedMentions) {
			options.data.allowed_mentions = this._formatAllowedMentions(
	...
	createThread(channelID, options, file) {
		if (options.message) {
			options.message.allowed_mentions = this._formatAllowedMentions(
				options.message.allowedMentions
			);
	...
	createThreadWithoutMessage(channelID, options) {
		...
		this.emit(
			'warn',
			'[DEPRECATED] createThreadWithoutMessage() is deprecated. Use createThread() instead.'
		);
		return this.requestHandler
			.request('POST', Endpoints.THREAD_WITHOUT_MESSAGE(channelID), true, {
				auto_archive_duration: options.autoArchiveDuration,
				invitable: options.invitable,
				name: options.name,
				type: options.type,
	...
	crosspostMessage(channelID, messageID) {
		return this.requestHandler
			.request('POST', Endpoints.CHANNEL_CROSSPOST(channelID, messageID), true)
			.then(message => new Message(message, this));
	}
	...
	deleteAutoModerationRule(guildID, ruleID, reason) {
		return this.requestHandler.request(
			'DELETE',
			Endpoints.AUTO_MODERATION_RULE(guildID, ruleID),
			true,
			{
				reason,
			}
	...
	deleteChannel(channelID, reason) {
		return this.requestHandler
			.request('DELETE', Endpoints.CHANNEL(channelID), true, {
				reason,
			})
			.then(channel => Channel.from(channel, this));
	}
	...
	deleteEmoji(emojiID) {
		return this.requestHandler.request(
			'DELETE',
			Endpoints.APPLICATION_EMOJI(this.application.id, emojiID),
			true
		);
	}
	...
	deleteGuildSoundboardSound(guildID, soundID, reason) {
		return this.requestHandler.request(
			'DELETE',
			Endpoints.SOUNDBOARD_SOUND_GUILD(guildID, soundID),
			true,
			{ reason }
		);
	}
	...
	deleteGuildTemplate(guildID, code) {
		return this.requestHandler
			.request('DELETE', Endpoints.GUILD_TEMPLATE_GUILD(guildID, code), true)
			.then(template => new GuildTemplate(template, this));
	}
	...
	deleteMessages(channelID, messageIDs, reason) {
		...
		if (messageIDs.length > 100) {
			return this.requestHandler
				.request('POST', Endpoints.CHANNEL_BULK_DELETE(channelID), true, {
	...
	editAutoModerationRule(guildID, ruleID, options) {
		return this.requestHandler
			.request('PATCH', Endpoints.AUTO_MODERATION_RULE(guildID, ruleID), true, {
				actions: options.actions,
				enabled: options.enabled,
				event_type: options.eventType,
				exempt_channels: options.exemptChannels,
	...
	editCommand(commandID, command) {
		...
		if (command.defaultPermission !== undefined) {
			...
			this.emit(
				'warn',
				'[DEPRECATED] editCommand() was called with a `defaultPermission` parameter. This has been replaced with `defaultMemberPermissions`'
			);
			command.default_permission = command.defaultPermission;
		}
		if (command.defaultMemberPermissions !== undefined) {
	...
	editCommandPermissions(guildID, commandID, permissions, reason) {
		...
		return this.requestHandler.request(
			'PUT',
			Endpoints.COMMAND_PERMISSIONS(this.application.id, guildID, commandID),
			true,
			{ permissions, reason }
		);
	}
	...
	editGuildCommand(guildID, commandID, command) {
		...
		if (command.defaultPermission !== undefined) {
			...
			this.emit(
				'warn',
				'[DEPRECATED] editGuildCommand() was called with a `defaultPermission` parameter. This has been replaced with `defaultMemberPermissions`'
			);
			command.default_permission = command.defaultPermission;
		}
			...
			.request('PATCH', Endpoints.GUILD_MEMBER(guildID, memberID), true, {
				roles:
					...
					options.roles.filter(
						(roleID, index) => options.roles.indexOf(roleID) === index
					),
				nick: options.nick,
				mute: options.mute,
				deaf: options.deaf,
	...
	editGuildMFALevel(guildID, level, reason) {
		return this.requestHandler.request(
			'POST',
			Endpoints.GUILD_MFA_LEVEL(guildID),
			true,
			{
				level,
				reason,
	...
	editGuildOnboarding(guildID, options, reason) {
		...
		return this.requestHandler.request(
			'PATCH',
			Endpoints.GUILD_ONBOARDING(guildID),
			true,
			options
		);
	}
	...
	editMessage(channelID, messageID, content) {
		...
		return this.requestHandler
			.request(
				...
				Endpoints.CHANNEL_MESSAGE(channelID, messageID),
				true,
				file ? { payload_json: content } : content,
				file
			)
			.then(message => new Message(message, this));
	}
	...
	editRole(guildID, roleID, options, reason) {
		...
		return this.requestHandler
			.request('PATCH', Endpoints.GUILD_ROLE(guildID, roleID), true, options)
	...
	editRoleConnectionMetadataRecords(data) {
		for (const connection of data) {
			if (connection.key !== undefined) {
				if (!/^[a-z0-9_]{1,50}$/.test(connection.key)) {
					throw new Error(
						'Role connection keys can only contain lowercase characters (a-z), numbers (0-9) or underscores (_)'
					);
				}
			}
		}
		return this.requestHandler.request(
			'PUT',
			Endpoints.ROLE_CONNECTION_METADATA_RECORDS(this.application.id),
			true,
			data
		);
	}
	...
	editRolePosition(guildID, roleID, position) {
		...
		roles = roles
			.filter(
				role =>
					min <= role.position && role.position <= max && role.id !== roleID
			)
			.sort((a, b) => a.position - b.position);
	...
	editStageInstance(channelID, options) {
		return this.requestHandler
			.request('PATCH', Endpoints.STAGE_INSTANCE(channelID), true, options)
			.then(instance => new StageInstance(instance, this));
	}
	...
	editWebhookMessage(webhookID, token, messageID, options) {
		...
		return this.requestHandler
			.request(
				...
				Endpoints.WEBHOOK_MESSAGE(webhookID, token, messageID),
				false,
				file ? { payload_json: options } : options,
				file
			)
	...
	endPoll(channelID, messageID) {
		return this.requestHandler
			.request('POST', Endpoints.POLL_END(channelID, messageID), true)
			.then(message => new Message(message, this));
	}
	...
	executeSlackWebhook(webhookID, token, options) {
		...
		return this.requestHandler.request(
			'POST',
			Endpoints.WEBHOOK_TOKEN_SLACK(webhookID, token) + (qs ? '?' + qs : ''),
			auth,
			options
		);
	}
	...
	executeWebhook(webhookID, token, options) {
		...
		return this.requestHandler
			.request(
				'POST',
				Endpoints.WEBHOOK_TOKEN(webhookID, token) + (qs ? '?' + qs : ''),
				!!options.auth,
				file ? { payload_json: options } : options,
				file
			)
	...
	followChannel(channelID, webhookChannelID, reason) {
		return this.requestHandler.request(
			...
			true,
			{ webhook_channel_id: webhookChannelID, reason: reason }
		);
	}
	...
	getAutoModerationRule(guildID, ruleID) {
		return this.requestHandler
			.request('GET', Endpoints.AUTO_MODERATION_RULE(guildID, ruleID), true)
			.then(rule => new AutoModerationRule(rule, this));
	}
	...
	getAutoModerationRules(guildID) {
		return this.requestHandler
			.request('GET', Endpoints.AUTO_MODERATION_RULES(guildID), true)
	...
	getEmojis() {
		return this.requestHandler
			.request('GET', Endpoints.APPLICATION_EMOJIS(this.application.id), true)
			.then(data => {
			...
			.then(data => {
				...
				return {
					entries: data.audit_log_entries.map(
						entry => new GuildAuditLogEntry(entry, this)
					),
	...
	getGuildAuditLogs(guildID, limit, before, actionType, userID) {
		return this.getGuildAuditLog.call(
			this,
			guildID,
			limit,
			before,
	...
	getGuildScheduledEvents(guildID, options = {}) {
		...
		return this.requestHandler
			.request('GET', Endpoints.GUILD_SCHEDULED_EVENTS(guildID), true, options)
			...
			.then(data =>
				data.map(eventUser => {
					...
					return {
						...
						member:
							eventUser.member && guild
								? guild.members.update(eventUser.member)
								: new Member(eventUser.member),
	...
	getGuildSoundboardSound(guildID, soundID) {
		return this.requestHandler
			.request('GET', Endpoints.SOUNDBOARD_SOUND_GUILD(guildID, soundID), true)
			.then(sound => new SoundboardSound(sound, this));
	}
	...
	getGuildSoundboardSounds(guildID) {
		return this.requestHandler
			.request('GET', Endpoints.SOUNDBOARD_SOUNDS_GUILD(guildID), true)
	...
	getGuildVoiceState(guildID, userID = '@me') {
		return this.requestHandler
			.request('GET', Endpoints.GUILD_VOICE_STATE(guildID, userID), true)
	...
	getGuildWidgetImageURL(guildID, style) {
		return (
			...
			Endpoints.BASE_URL +
			Endpoints.GUILD_WIDGET_IMAGE(guildID) +
	...
	getMessage(channelID, messageID) {
		return this.requestHandler
			.request('GET', Endpoints.CHANNEL_MESSAGE(channelID, messageID), true)
			.then(message => new Message(message, this));
	}
	...
	getMessageReaction(
		...
		if (options.before) {
			...
			this.emit(
				'warn',
				'[DEPRECATED] getMessageReaction() was called with a `before` parameter. Discord no longer supports this parameter'
			);
		}
		return this.requestHandler
			.request(
	...
	async getMessages(channelID, options = {}, before, after, around) {
		...
		if (limit && limit > 100) {
			...
			const get = async (_before, _after) => {
				...
				logs = _after
					? messages.map(message => new Message(message, this)).concat(logs)
					: logs.concat(messages.map(message => new Message(message, this)));
	...
	getPollAnswerVoters(channelID, messageID, answerID, options = {}) {
		return this.requestHandler
			.request(
				'GET',
				Endpoints.POLL_ANSWER_VOTERS(channelID, messageID, answerID),
				true,
				{
					after: options.after,
					limit: options.limit,
	...
	getRESTGuildRole(guildID, roleID) {
		...
		return this.requestHandler
			.request('GET', Endpoints.GUILD_ROLE(guildID, roleID), true)
			.then(role => new Role(role, { shard: { client: this } }));
	}
	...
	getSoundboardSounds() {
		return this.requestHandler
			.request('GET', Endpoints.SOUNDBOARD_SOUNDS_DEFAULT, true)
			.then(sounds => sounds.map(sound => new SoundboardSound(sound, this)));
	}
	...
	getThreadMember(channelID, userID, withMember) {
		return this.requestHandler
			.request('GET', Endpoints.THREAD_MEMBER(channelID, userID), true, {
	...
	getWebhookMessage(webhookID, token, messageID) {
		return this.requestHandler
			.request('GET', Endpoints.WEBHOOK_MESSAGE(webhookID, token, messageID))
			.then(message => new Message(message, this));
	}
	...
	joinVoiceChannel(channelID, options = {}) {
		...
		if (
			channel.guild &&
			channel.guild.members.has(this.user.id) &&
			!(
				channel.permissionsOf(this.user.id).allow &
				Constants.Permissions.voiceConnect
			)
		) {
			return Promise.reject(
				new Error('Insufficient permission to connect to voice channel')
			);
		}
		this.shards
			.get(this.guildShardMap[this.channelGuildMap[channelID]] || 0)
			.sendWS(Constants.GatewayOPCodes.VOICE_STATE_UPDATE, {
	...
	sendSoundboardSound(channelID, options) {
		...
		return this.requestHandler.request(
			'POST',
			Endpoints.SOUNDBOARD_SOUNDS_SEND(channelID),
			true,
			options
		);
	}
	...
	setVoiceChannelStatus(channelID, status, reason) {
		return this.requestHandler.request(
			'PUT',
			Endpoints.CHANNEL_VOICE_STATUS(channelID),
			true,
			{
				status,
				reason,
